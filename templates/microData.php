    ...
    $('.check_boxMicro').each(function(){
      ...
      else
      {
        html += '<td><select name="ec_result[]" id="ec_result_' 
           + $(this).attr('id')
           + '" class="form-control input-sm" required><option value="Positive">Positive</option><option value="Negative">Negative</option></select></td>';
      }
      ...
      if($(this).data('remarks') == 'Printed' && (usertp == 'LAB-MICRO-HEAD' || usertp == 'LAB-MICRO-USER'))
      {
        html += '<td><input type="text" name="micro_analyst[]" id="micro_analyst_' 
             + $(this).attr('id')
             + '" class="form-control input-sm" onkeypress="return false;" onKeyDown="preventBackspace();" autocomplete="off" placeholder="Please Select Analyst" readonly></td>';
      }
      // note: can't put readonly with required attri. Name of analyst will not be added
      else
      {
        html += '<td><input type="text" name="micro_analyst[]" id="micro_analyst_' 
             + $(this).attr('id')
