...
function addMicro($sam_param_w, $sample_label, $water_des, $sam_point, $sam_place, $tc_potable, $thc_potable,
                  $hpc_result, $ec_result, $tc_waste, $fc_waste, $remarks, $req_id)
{
        $pdo = "SELECT * FROM micro_table WHERE sam_param=:sam_param
                                          AND sample_label=:sample_label 
                                          AND water_des=:water_des
                                          AND sam_point=:sam_point
                                          AND sam_place=:sam_place
                                          AND req_id=:req_id";
        $db = getConnection();
        $stmt = $db->prepare($pdo);
        $stmt->bindValue(':sam_param', $sam_param_w);         
        $stmt->bindValue(':sample_label', $sample_label); 
        $stmt->bindValue(':water_des', $water_des);
        $stmt->bindValue(':sam_point', $sam_point);
        $stmt->bindValue(':sam_place', $sam_place);
        ...
        {   
          ...
          $query = "INSERT INTO `micro_table` (`sam_param`, `sample_label`, `water_des`, `sam_point`,
                                               `sam_place`, `tc_potable`, `thc_potable`, `hpc_result`,
                                               `ec_result`, `tc_waste`, `fc_waste`, `remarks`, `req_id`) 
                      VALUES (:sam_param, :sample_label, :water_des, :sam_point, :sam_place,
                              :tc_potable, :thc_potable, :hpc_result, :ec_result, :tc_waste, :fc_waste,
                              :remarks, :req_id)";
            try {
                $db         = getConnection();
...
function getMicroResultByID($micro_id){
...
function getMicroResult($req_id){
    $query = "SELECT clients_table.company_name, clients_table.add_st, clients_table.add_city, 
                     ...
                     micro_table.sam_point, micro_table.sam_place, micro_table.thc_potable,
...
function updateMicro($sam_param_w, $sample_label, $water_des, $sam_point, $sam_place, $tc_potable,
